...
def solveOutputPredicate_If_Sub(subst, pred, docXML, posMut):
    ...
    newPred = createBinaryPred(condition, newPred, docXML)
    if subst.lastChild.previousSibling.tagName == "Else":
        ...
        newPred = createNaryPred(newPred, binaryPredElse, docXML)
    return newPred

def solveOutputPredicate_Select(subst, pred, docXML, posMut):
    '''
    ...
    for childnode in subst.childNodes:
        if childnode.nodeType != childnode.TEXT_NODE:
            if childnode.tagName == "When_Clauses":
                for when in childnode:
                    None
            ...
            if childnode.tagName != 'Attr':
                ...
                if FinalPred.hasChildNodes():
                    if FinalPred.firstChild.nextSibling.tagName == "Nary_Sub":
                        ...
                        naryNode = createNaryPred(FinalPred.firstChild.nextSibling, newPred, docXML)
...
def makePredicateInstruction(subst, pred, posMut, docXML):
    ...
    if subst.tagName == "Select":
        pred = solveOutputPredicate_Select(subst, pred, docXML, posMut)
    return pred
