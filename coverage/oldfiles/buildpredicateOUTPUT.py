...
def makePredicateXML(node, predicateXML, way, path, inputs, docXML, posMut, operationImp, importedMch, operationName, impName, outputXML):
    '''
    ...
    if " ENDWHILE" in buildpaths.graphgen.nodecond[node]:
        ...
        predicateXML, outputXML = buildInstruction(node, predicateXML, docXML, posMut, outputXML)
    elif buildpaths.graphgen.nodetype[node] == "Call":
        if predicateXML.hasChildNodes():
            if outputXML.hasChildNodes():
                predicateXML, outputXML = solveroc.buildOperationCallWithOutput(node, predicateXML, outputXML, docXML, operationImp,
            ...
            if outputXML.hasChildNodes():
                outputXML = solveroc.buildOperationCall(node, outputXML.cloneNode(10), docXML, operationImp, importedMch, operationName, impName, posMut)
    elif buildpaths.graphgen.nodetype[node] == "END":
        outputXML.appendChild(docXML.createTextNode('\n'))
        outputXML.appendChild(buildpaths.graphgen.nodedata[node].cloneNode(10))
        outputXML.appendChild(docXML.createTextNode('\n'))
    return predicateXML, posMut, outputXML
...
def buildInstruction(node, predicateXML, docXML, posMut, outputXML):
    '''
    ...
    if outputXML.hasChildNodes():
        ...
        values = buildpaths.graphgen.nodedata[node].getElementsByTagName("Values")[0]
        values = values.firstChild.nextSibling
        allId = outputXML.getElementsByTagName("Id")
        for Id in allId:
            if Id.getAttribute("value") == variablesId:
                if Id.getAttribute("value") in posMut:
                    parent = Id.parentNode
