...
def make_id(node, i = 0):
    """Build an ID in a string"""
    text = ""
    if i == 0:
        text += str(node.getAttribute("value"))  # Get the value inside the ID
    else:
        if node.parentNode.tagName == "Variables":
            IdNodeList = node.parentNode.getElementsByTagName("Id")
            text += ",  "+str(IdNodeList[i].getAttribute("value"))
        if node.parentNode.tagName == "Values":
            IntegerNodeList = node.parentNode.getElementsByTagName("Integer_Literal")
            text += ",  "+str(IntegerNodeList[i].getAttribute("value"))
    return text
...
def make_inst(node):
    ...
    for i in range(node.childNodes.length):
        if node.childNodes.item(i).nodeType != node.childNodes.item(i).TEXT_NODE:
            ...
            text += callmake(node, tag, int(i / 2))
    return text
...
# def make_initialisation(node, inLine = False):
#     if inLine:
#         text = node.tagName.upper() + "\n    "
#     else:
#         text = ""
