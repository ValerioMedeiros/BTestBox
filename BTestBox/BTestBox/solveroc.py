...
def buildOperationCallInsideWhile(node, predicateXML, docXML, operationImp, importedMch, operationName, impName, posMut,
    ...
    if calledOperation is None:
        ...
        if hasWhile == True:
            ...
            predicateXML.replaceChild(output.firstChild.firstChild.nextSibling.lastChild.previousSibling.cloneNode(10),
                                      predicateXML.firstChild.nextSibling)
    os.remove(directory+ os.sep +"encodeddocumentinput.xml")
    os.remove(directory+ os.sep +"encodeddocumentoutput.xml")
    os.remove(directory+ os.sep +""+impName + ".ibxml")
...
def getOperationIBXML(impName, operationName, calledOperation, operationInputs, operationOutputs,
    ...
    """
    args = [atelierBDir+ os.sep +'bbin'+ os.sep +'win32'+ os.sep + 'pog.exe']
    args.append("-i")
    args.append(directory+ os.sep +''+impName+".bxml")
    subprocess.call(args)
    ibxml = minidom.parse(directory+ os.sep +''+impName+".ibxml")
...
def make_Sub_Calculus(calledOperation, predicateXML, directory, atelierBDir):
    ...
    f = open(directory+ os.sep +'encodeddocumentinput.xml', 'bw')
    f.write(encodeddocument)
    f.close()
    args.append(atelierBDir+ os.sep +'bbin' + os.sep + 'win32'+ os.sep + 'substitution_calculus_pred.exe')
    args.append(directory+ os.sep +'encodeddocumentinput.xml')
    ...
    if p.returncode == 0:
        ...
        # print(output)
        print(errors)
    f = open(directory + os.sep +'encodeddocumentoutput.xml', 'bw')
    f.write(output)
    f.close()
    outputXML = minidom.parse(directory+ os.sep +"encodeddocumentoutput.xml")
    return outputXML
